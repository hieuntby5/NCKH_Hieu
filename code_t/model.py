...
class SNN_VGG9_BNTT(nn.Module):
    ...
    def forward(self, inp):
        ...
        mem_conv3 = torch.zeros(batch_size, 32, self.img_size//2, self.img_size//2).cuda()
        mem_conv4 = torch.zeros(batch_size, 32, self.img_size//2, self.img_size//2).cuda()
        mem_conv5 = torch.zeros(batch_size, 32, self.img_size//4, self.img_size//4).cuda()
        mem_conv6 = torch.zeros(batch_size, 32, self.img_size//4, self.img_size//4).cuda()
        ...
        for t in range(self.num_steps):
            ...
            for i in range(len(self.conv_list)):
                mem_conv_list[i] =  mem_conv_list[i] + self.bntt_list[i](self.conv_list[i](out_prev))
            ...
            mem_fc1 =  mem_fc1 + self.bntt_fc(self.fc1(out_prev))
...
class SNN_VGG11_BNTT(nn.Module):
    def __init__(self, num_steps, leak_mem=0.95, img_size=32,  num_cls=100):
        ...
        self.bntt3 = nn.BatchNorm2d(256, eps=1e-4, momentum=0.1, affine=affine_flag)
        self.conv4 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.bntt4 = nn.BatchNorm2d(256, eps=1e-4, momentum=0.1, affine=affine_flag)
        self.pool3 = nn.AvgPool2d(kernel_size=2)

        self.conv5 = nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.bntt5 = nn.BatchNorm2d(512, eps=1e-4, momentum=0.1, affine=affine_flag) 
        self.conv6 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.bntt6 = nn.BatchNorm2d(512, eps=1e-4, momentum=0.1, affine=affine_flag) 
        ...
        self.bntt_fc = nn.BatchNorm1d(4096, eps=1e-4, momentum=0.1, affine=affine_flag) 
    ...
    def forward(self, inp):
        ...
        for t in range(self.num_steps):
            ...
            for i in range(len(self.conv_list)):
                mem_conv_list[i] = self.leak_mem * mem_conv_list[i] + self.bntt_list[i](self.conv_list[i](out_prev))
                mem_thr = (mem_conv_list[i] / self.conv_list[i].threshold) - 1.0
                out = self.spike_fn(mem_thr)
                rst = torch.zeros_like(mem_conv_list[i]).cuda()
                rst[mem_thr > 0] = self.conv_list[i].threshold
            ...
            mem_fc1 = self.leak_mem * mem_fc1 + self.bntt_fc(self.fc1(out_prev))
            mem_thr = (mem_fc1 / self.fc1.threshold) - 1.0
            out = self.spike_fn(mem_thr)
            rst = torch.zeros_like(mem_fc1).cuda()
            rst[mem_thr > 0] = self.fc1.threshold
